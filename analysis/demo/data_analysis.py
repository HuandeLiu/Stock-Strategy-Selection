...
def average_analysis(df):
    ...
    if(len(success) > 0):
        ...
        if(date_difference.days < happen):
            ...
            with open('./data/average_analysis{}.csv'.format(datetime.now().strftime("%Y%m%d")), 'a') as file:
                file.write(lines)
                file.write("\n")
...
"""
def Boring_market_trends(df):
    # df =df[['open','close','changed']]
    min_days = 4
    var_threshold = 0.0005 # 方差波动在1%内
    change_threshold = 0.02 # 波动率低阈值 1% ,最大是0.5
    history = 10 # 回往历史窗口
    patience = 1
    # df = df.iloc[len(df) - history:]
    if(len(df)<history):
        return
    # 1、找出波动率低的索引日期
    ...
    if(flag):
        print(f"股票代码{df['ts_code'].iloc[0]},在{start_date}到{end_date}时间内存在涨跌幅小，波动率低")
        lines = ','.join([df['ts_code'].iloc[0], start_date, end_date])
        with open('./data/Boring_market_trends{}.csv'.format(datetime.now().strftime("%Y%m%d")), 'a') as file:
            file.write(lines)
            file.write("\n")
        # print(return_data)
...
"""
def find_gap_down_recovery_stocks(df):
    history = 10
    ...
    if(flag):
        print(f"股票代码{df['ts_code'].iloc[0]},在{start_date}到{end_date}时间内跳空低开，然后回补缺口")
        lines = ','.join([df['ts_code'].iloc[0],start_date,end_date])
        with open('./data/find_gap_down_recovery_stocks{}.csv'.format(datetime.now().strftime("%Y%m%d")), 'a') as file:
            file.write(lines)
            file.write("\n")
...
def trading_volume_trend(df):
    ...
    if(len(df) < history):
        return
    # 1、区分出红绿成交量
    indices = df['pre_close'] < df['close'] # 为True则为红，为False则为绿
    # print(indices)
    ...
    for i in range(0,len(df) - min_windows):
        ...
        flag_red = False
        flag_green = False
        if(all(indices.iloc[i : i + min_windows])):
        ...
        elif(all(~indices.iloc[i : i + min_windows])):
    ...
    if(len(success) >= period):
        ...
        data.to_csv('./data/trading_volume_trend{}.csv'.format(datetime.now().strftime("%Y%m%d")),mode = 'a',header=False,index=False)
        # print(success)
        # print(",".join(success))
...
def testsingle(ts_code):
    ...
    # 加载最近100个交易日的数据
    df_single = df[df['ts_code'] == ts_code].sort_values(by='trade_date').iloc[-100:, :].reset_index(drop=True)
    print(df_single.iloc[80:])
