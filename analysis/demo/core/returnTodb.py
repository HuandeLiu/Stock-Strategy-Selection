...
import logging
logging.basicConfig(filename='/vms/sdb/lhd/code/paper/stock/analysis/demo/core/returnTodb.log', filemode='a', level=logging.INFO,
                    format='[%(levelname)s][%(asctime)s][%(name)s]%(message)s', datefmt='%Y-%m-%d %H:%M:%S')
logger = logging.getLogger("returnTodb")
def insert_data(datas,op):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            # 定义批量插入的 SQL 语句
            if op == 0:
                sql = ("INSERT INTO `operator` VALUES (%s,%s, %s, %s,%s,%s)")
            if op == 1: # 均值分析
